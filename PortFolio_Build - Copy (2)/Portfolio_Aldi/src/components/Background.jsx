  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      blobRefs.current.forEach((blob, index) => {
        if (blob) {
          // Pastikan blob tidak null
          const initialPos = initialPositions[index];

          // Perhitungan pergerakan X dan Y
          const xOffset = Math.sin(newScroll / 100 + index * 0.5) * 340; // Gerakan horizontal
          const yOffset = Math.cos(newScroll / 100 + index * 0.5) * 40; // Gerakan vertikal

          const x = initialPos.x + xOffset;
          const y = initialPos.y + yOffset;

          // Terapkan transformasi dengan transisi yang halus
