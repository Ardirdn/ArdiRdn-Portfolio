...
import { assetPath, transformAssetPaths } from '../utils/assetPath';

export const fetchProjects = async () => {
    try {
        const response = await fetch(assetPath('/data/projects.json'));
        if (!response.ok) {
            throw new Error('Failed to fetch projects');
        }
        const projects = await response.json();
        return transformAssetPaths(projects);
    } catch (error) {
...
export const fetchTools = async () => {
    try {
        const response = await fetch(assetPath('/data/tools.json'));
        if (!response.ok) {
            throw new Error('Failed to fetch tools');
        }
        const tools = await response.json();
        return transformAssetPaths(tools);
    } catch (error) {
...
export const fetchLevelDesigns = async () => {
    try {
        const response = await fetch(assetPath('/data/leveldesigns.json'));
        if (!response.ok) {
            throw new Error('Failed to fetch level designs');
        }
        const levelDesigns = await response.json();
        return transformAssetPaths(levelDesigns);
    } catch (error) {
...
export const fetch3DShowcase = async () => {
    try {
        const response = await fetch(assetPath('/data/3dshowcase.json'));
        if (!response.ok) {
            throw new Error('Failed to fetch 3d showcase');
        }
        const data = await response.json();
        return transformAssetPaths(data);
    } catch (error) {
